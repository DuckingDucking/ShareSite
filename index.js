...
app.post("/api/submit", async (req, res) => {
  ...
  if (!cookie || !url || !amount || !interval) {
    return res
      .status(400)
      .json({ error: "Missing state, url, amount, or interval" });
  }
  try {
...
async function startShareSession(cookies, url, amount, interval) {
  ...
  if (!id) {
    throw new Error(
      "Unable to get link id: invalid URL, it's either a private post or visible to friends only",
    );
  }
...
async function convertCookie(cookie) {
  try {
    ...
    if (!sbCookie)
      throw new Error("Invalid appstate. Please provide a valid appstate.");
    const sbValue = sbCookie.value;
    return `sb=${sbValue}; ${cookies
      .slice(1)
      .map((c) => `${c.key}=${c.value}`)
      .join("; ")}`;
  } catch {
    throw new Error(
      "Error processing appstate. Please provide a valid appstate.",
    );
  }
}
